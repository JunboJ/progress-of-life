...
const getCellPosition = (
	...
	const y =
		row === 0
			...
			: (calendarStyle.cellHeight + calendarStyle.cellGap) * row + calendarStyle.paddingTop;
	return { x, y, row, col };
};
...
export const getOutlineBounds = (
	...
	if (startPos.row === endPos.row) {
		...
		const lastCellFirstRow = getCellPosition(
			startPos.row * dimensions.numberOfCols + dimensions.numberOfCols - 1,
			dimensions,
			calendarStyle
		);
		bounds.push({
		...
		for (let row = startPos.row + 1; row < endPos.row; row++) {
			...
			const lastCellOfRow = getCellPosition(
				row * dimensions.numberOfCols + dimensions.numberOfCols - 1,
				dimensions,
				calendarStyle
			);
			bounds.push({
