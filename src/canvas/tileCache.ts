...
const drawCellsInRegion = (
	...
	const startCol = Math.max(0, Math.floor((regionLeft - layout.computedPaddingLeft) / layout.colStride));
	const endCol = Math.min(
		layout.numberOfCols - 1,
		Math.ceil((regionRight - layout.computedPaddingLeft) / layout.colStride)
	);
	const startRow = Math.max(0, Math.floor((regionTop - layout.paddingTop) / layout.rowStride));
	const totalRows = Math.ceil(layout.totalCells / layout.numberOfCols);
	const endRow = Math.min(totalRows - 1, Math.ceil((regionBottom - layout.paddingTop) / layout.rowStride));
